...
func (repo *NacosTcInstanceRepository) Get(id string) (instance TcInstance, err error) {
	...
	req.QueryType = tccommon.StringPtr("nacos")
	req.QuerySource = tccommon.StringPtr("cloud_metrics")
...
func (repo *NacosTcInstancePodRepositoryImpl) GetNacosPodInfo(instanceId string) (*sdk.DescribeNacosReplicasResponse, error) {
	...
	req.InstanceId = tccommon.StringPtr(instanceId)
	return repo.client.DescribeNacosReplicas(req)
}
func NewNacosTcInstancePodRepository(cred common.CredentialIface, c *config.TencentConfig, logger log.Logger) (NacosTcInstancePodRepository, error) {
